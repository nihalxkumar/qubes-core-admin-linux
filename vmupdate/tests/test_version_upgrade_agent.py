...
def test_version_upgrade_refuses_non_redhat_qube():
    mgr = make_dnf_cli()
    with patch.object(mgr, "run_cmd") as run_cmd, patch(
        "source.dnf.dnf_cli.get_os_data",
        return_value={"id": "debian", "os_family": "Debian", "release": "12"},
    ):
        code = mgr.version_upgrade("42")

    assert code == EXIT.ERR_VM_UPDATE
    run_cmd.assert_not_called()  # bail before anything irreversible
